...
export default function CosmicDust({
  particleCount = 150, // Balanced default count for rich density and high performance
  speedMultiplier = 1.0,
  particleSize = 2.5,
  theme = "system",
}: CosmicDustProps) {
  ...
  useEffect(() => {
    ...
    const resizeObserver = new ResizeObserver((entries) => {
      for (const entry of entries) {
        const w = entry.contentRect.width || canvas.offsetWidth || window.innerWidth;
        const h = entry.contentRect.height || canvas.offsetHeight || window.innerHeight;
    ...
    const animate = (t: number) => {
      ...
      particles.forEach((p, index) => {
        ...
        // Store trail history (increased to 8 for smoother, more visible trails)
        ...
        // Smoothly reduce opacity in the center content column to avoid overlapping with text
        const center = width / 2;
        const halfContentWidth = width > 800 ? 380 : width * 0.4;
        const distanceFromCenter = Math.abs(p.x - center);
        
        let opacityMultiplier = 1.0;
        if (distanceFromCenter < halfContentWidth) {
          const t = distanceFromCenter / halfContentWidth;
          opacityMultiplier = 0.15 + 0.85 * Math.pow(t, 2); // Soft quadratic curve for clear reading area
        }
        ...
        if (p.history.length > 1) {
          ...
          ctx.strokeStyle = `${p.color}${p.opacity * 0.55 * opacityMultiplier})`;
          ctx.lineWidth = p.size * 0.6;
          ctx.lineCap = "round";
          ctx.stroke();
        }

        // Draw particle core (removed heavy shadowBlur calculations for high-performance rendering)
        ctx.beginPath();
        ctx.arc(p.x, p.y, p.size, 0, Math.PI * 2);
        ctx.fillStyle = `${p.color}${p.opacity * opacityMultiplier})`;
        ctx.fill();
      });
    };
  ...
  return <canvas ref={canvasRef} className="w-full h-full fixed inset-0 -z-10 pointer-events-none" />;
}
